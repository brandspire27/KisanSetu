        ...
        <>
          ...
          {incomingOrders.map(order => (
            <div key={order._id} className="card shadow p-3 mb-3">
              ...
              <span className={`badge ${order.status === "accepted" ? "bg-success" : order.status === "rejected" ? "bg-danger" : "bg-warning text-dark"}`}>
                {order.status.toUpperCase()}
              </span>
              {order.status === "pending" && (
                <div className="mt-2">
                  <button className="btn btn-success me-2" onClick={() => updateOrderStatus(order._id, "accepted")}>Accept</button>
                  <button className="btn btn-danger" onClick={() => updateOrderStatus(order._id, "rejected")}>Reject</button>
                </div>
              )}
            </div>
          ))}
          ...
          {showAddProduct && (
            <div className="card shadow p-4 mb-4">
              ...
              <input className="form-control mb-2" placeholder="Price" type="number" value={productPrice} onChange={(e) => setProductPrice(e.target.value)} />
              <input className="form-control mb-2" placeholder="Quantity" type="number" value={productQuantity} onChange={(e) => setProductQuantity(e.target.value)} />
              <input className="form-control mb-3" placeholder="Image URL" value={image} onChange={(e) => setImage(e.target.value)} />
